...
#include "../model/JsonUnserializer.h"
#include "../model/JsonSerializer.h"
#include "../model/HttpErrors.h"
...
public:
    ...
    pplx::task<std::shared_ptr<T>> request(const web::http::method &mtd, web::uri_builder uri, U bodyData) {
        ...
        return client.request(mtd, uri.to_string(), json.serialize(), "application/json").then([=](web::http::http_response response) {
            return onRequest<T>(response);
        });
    }

    template <typename T>
    std::shared_ptr<T> onRequest(web::http::http_response response) {
        auto json = response.extract_json().get();
        auto s = JSonUnserializer{json};
        switch (response.status_code()) {
            case 200:
                return s.unserialize<std::shared_ptr<T>>();
            case 401:
