...
GigaApi::GroupsApi::createGroup (const string_t& name) const
{
    ...
    return api._client.request<Group> (methods::POST, uri, std::move(body));
}

pplx::task<std::shared_ptr<Group>>
GigaApi::GroupsApi::getGroupById (int64_t groupId) const
{
    auto uri = api._client.uri (U("groups"), groupId);
...
GigaApi::GroupsApi::updateGroupName (int64_t groupId, const string_t& name) const
{
    ...
    auto body = JsonObj{};
    body.add (U("name"), name);
    return api._client.request<Group> (methods::PUT, uri, std::move(body));
}
...
{
    auto uri = api._client.uri (U("groups"), groupId);
    return api._client.request<Empty> (methods::DEL, uri);
}

pplx::task<std::shared_ptr<Group>>
GigaApi::GroupsApi::shareNodeWithGroup (int64_t groupId, const string_t& nodeId) const
{
    auto uri = api._client.uri (U("groups"), groupId, U("nodes"), nodeId);
    return api._client.request<Group> (methods::PUT, uri);
}
...
{
    auto uri = api._client.uri (U("groups"), groupId, U("nodes"), nodeId);
    return api._client.request<Group> (methods::DEL, uri);
}

pplx::task<std::shared_ptr<Group>>
GigaApi::GroupsApi::addUserToGroup (int64_t groupId, uint64_t userId) const
{
    auto uri = api._client.uri (U("groups"), groupId, U("users"), userId);
    return api._client.request<Group> (methods::PUT, uri);
}
