#define BOOST_TEST_MODULE crypto
#include <boost/test/included/unit_test.hpp>

#include <giga/utils/Crypto.h>

using namespace boost::unit_test;
using namespace giga;
using utility::string_t;

BOOST_AUTO_TEST_CASE(test_calculate_fid)
{
    auto fid = Crypto::calculateFid(U("177abc9bcd3bc9785b96e06fcf63d82c58b6f8f6"));
	BOOST_CHECK(U("MUr243SzLSVf11/c7T0SZqyf") == fid);
}

BOOST_AUTO_TEST_CASE(test_calculate_fkey)
{
    auto fkey = Crypto::calculateFkey(U("177abc9bcd3bc9785b96e06fcf63d82c58b6f8f6"));
	BOOST_CHECK(U("V9leIEW8PYNMhlDuMNvkpWei") == fkey);
}

BOOST_AUTO_TEST_CASE(test_aes)
{
    auto result    = Crypto::aesEncrypt(U("password"), U("data"));
    auto decrypted = Crypto::aesDecrypt(U("password"),
                                        std::get<0>(result),
										std::get<1>(result),
										std::get<2>(result));

	BOOST_CHECK(U("data") == decrypted);
}

BOOST_AUTO_TEST_CASE(test_load_rsa_keys)
{
    auto privateKey     = string_t(U(R"()"));
    auto publicKey      = string_t(U(R"(MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCvupt+SoaJxTnyvzo7d2xtVvhe5N2qxhyW/OVNuh/OeWO2T+BZPJsh6n3+YV4mulsAxmbUvvwpL9EI3xynjw5WpSsXgNAg+s2mAbyGTAOLAlPlymBy1R57XA0DgnIfEh3mQsiWP9uaXp8V8PNkWNC6d+JtRUEImDFy+mFI5XIcQwIDAQAB)"));

    auto r = Rsa{publicKey, privateKey};
    auto encrypted = r.encrypt(U("test"));
	BOOST_CHECK(U("test") == r.decrypt(encrypted));
}

BOOST_AUTO_TEST_CASE(test_decrypt_nodekey)
{
    auto password       = U(R"(gigatribe)");
    auto passwordSalt   = U(R"(FLOhgPcpK+IzLOqqzUw2Dbe3o47IsDitc2DBQNiU0i8=)");
    auto privateKey     = string_t(U(R"()"));
    auto privateKeyEnc  = string_t(U(R"()"));
    auto publicKey      = string_t(U(R"(MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCvupt+SoaJxTnyvzo7d2xtVvhe5N2qxhyW/OVNuh/OeWO2T+BZPJsh6n3+YV4mulsAxmbUvvwpL9EI3xynjw5WpSsXgNAg+s2mAbyGTAOLAlPlymBy1R57XA0DgnIfEh3mQsiWP9uaXp8V8PNkWNC6d+JtRUEImDFy+mFI5XIcQwIDAQAB)"));
    auto nodeKeyEnc     = U(R"(HNhyBaoz42\/FFPKpI+YDoxeKLEyB0Pvwai4Fpo49AiqPegdSjqQMaxmtZO\/HtWX48M65QnRcxF6AEcKMQqxQCy4NibgpN61rcpXmcOXNc9recE\/D\/lPdLys9nT5Wrtn7BuwPpWd9vOu0dIzmkNO2ReAcxBedyT4zPXGtOxi+zms=)");
    auto nodeKey        = U("vohbW5WRs048GOIarXjchsymnSRPHvx9UuxHu8N9q3g=");
    auto aesIv          = U(R"(2yTS4KO5M14ix34g1aUFhA==)");
    auto aesSalt        = U(R"(NRxHK0GO+yA=)");

    auto masterKey = Crypto::calculateMasterPassword(passwordSalt, password);

    auto pkey = Crypto::aesDecrypt(masterKey, Crypto::base64decode(aesSalt), Crypto::base64decode(aesIv), Crypto::base64decode(privateKeyEnc));
	BOOST_CHECK(privateKey == pkey);

    auto rsa = Rsa{publicKey, pkey};
    auto nKey = rsa.decrypt(Crypto::base64decode(nodeKeyEnc));
	BOOST_CHECK(44 == nKey.size());
    if (nKey.size() < 44) {
        nKey = Crypto::base64encode(nKey);
    }
	BOOST_CHECK(nodeKey == nKey);
}

BOOST_AUTO_TEST_CASE(test_uncipher_keys)
{
    auto login          = U(R"(mobiuser01)");
    auto password       = U(R"(azertyuiop)");
    auto passwordSalt   = U(R"(3Xu9EtDPsnyYPjaNPKcbeACoM9chz9+lUdNxMBCDg2A=)");
    auto privateKeyEnc  = string_t(U(R"()"));
    auto privateKey     = string_t(U(R"()"));
    auto publicKey      = string_t(U(R"(MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCQq2qRyEYgOTkCSl99P+IcMSpMDNOvVve71Mh03C9sjo69mHNMP7qKTsBNv+ZepQTpBYy6bb23gmTd+LbjyOG8nVUrhkbWKjYeyOcQLuu8aecab6sxVsV/6qtNp7LuXkXDE+gHE1UdUT2iDmO2ump67PEf4CO41bbrVc1wB5mK0QIDAQAB)"));
    auto nodeKeyEnc     = U(R"(JwXYePKScBfV7zOaT3EnVvOPMONTT1yhjqnyBs8pngnjc7dr2aJOF0QJrC3kCFQ8IfArGqxeQC8Hi52nTtrTrege4DYdbZkJIM2ckaBqMlc613pVbUz6AdX3qZ/q9WfNXbbbE0bJdLXBstSVMsBb86sed1gVvR5JkrOuPsnYBzY=)");
    auto nodeKey        = U("asmiWz0ITE17vX4vWJ9Zw0ud581DBJ43RBBIdHBmeBU=");
    auto aesIv          = U(R"(Sc37HJg+jZLMtPq9UCmx7w==)");
    auto aesSalt        = U(R"(J/wabyBG4WA=)");

    auto loginPassword = Crypto::calculateLoginPassword(login, password);
	BOOST_CHECK(U("Ju51bwKeziurk32HMdVx8g==") == loginPassword);

    auto masterKey = Crypto::calculateMasterPassword(passwordSalt, password);
	BOOST_CHECK(U("+Q5WdxgwsDFC8/6JgK2JPA==") == masterKey);

    auto pkey = Crypto::aesDecrypt(masterKey, Crypto::base64decode(aesSalt), Crypto::base64decode(aesIv), Crypto::base64decode(privateKeyEnc));
	BOOST_CHECK(privateKey == pkey);

    auto rsa = Rsa{publicKey, pkey};
    auto nKey = rsa.decrypt(Crypto::base64decode(nodeKeyEnc));
	BOOST_CHECK(32 == nKey.size());
    if (nKey.size() < 44) {
        nKey = Crypto::base64encode(nKey);
    }
	BOOST_CHECK(nodeKey == nKey);
}
